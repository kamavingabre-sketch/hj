...
async function startBot() {
  ...
  if (!sock.authState.creds.registered) {
    ...
    // ── Railway: baca dari env var, bukan readline interaktif ──
    let phoneNumber = process.env.PHONE_NUMBER || '';
    if (!phoneNumber) {
      logger.error('PAIR', 'PHONE_NUMBER env var tidak ditemukan!');
      logger.error('PAIR', 'Set di Railway → Variables: PHONE_NUMBER=628xxxxxxxxxx');
      process.exit(1);
    }
